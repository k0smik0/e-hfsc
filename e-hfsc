...
# i.e. "tcp:24650-24690:c" means "tcp ports from 24650 to 24690 as client", while "tcp,udp" means together, and "s" means "server", etc
# "c" and "s" are optionals: if specified, they are used only, respectively, "c" for destination traffic (acting as client), "s" for source (acting as server)
# so, if you have a server listening on tcp 12345 port, use tcp:12345:s, while if you have a http server, use tcp:80,443:cs, since your server listens on 80 and 443 but your box is also probably a client for external http server (hence "c")
...
#SHAPER_OUTPUT_PRE="SHAPER_OUTPUT_PRE"
IPTABLES_EGRESS_CHAIN="POSTROUTING"
#IPTABLES_EGRESS_CHAIN_PRE="PREROUTING"

SHAPER_INGRESS="SHAPER_INGRESS"
#SHAPER_INPUT_PRE="SHAPER_INPUT_PRE"
...
#  set_rules_for_interface $DEV $SHAPER_EGRESS $IPTABLES_EGRESS_CHAIN $IPTABLES_EGRESS_CHAIN_PRE $UPLINK $1;
...
function set_tc_rules_for_interface() {
  ...
  ${TC} class add dev $dev parent 1:1  classid 1:"$CLASS_LOW_LATENCY" hfsc \
     ...
     ul rate ${link}kbit
     #      rt m1 ${link}kbit d  50ms m2 $((9*$link/10))kbit \
  ...
  ${TC} class add dev $dev parent 1:1  classid 1:"$CLASS_BROWSING" hfsc \
    sc m1 $((6*$link/10))kbit d 1s m2 $((1*$link/10))kbit \
    ul rate ${link}kbit
  ...
  ${TC} class add dev $dev parent 1:1  classid 1:"$CLASS_DEFAULT" hfsc \
    sc m1         0 d    2s m2 $((1*$link/20))kbit \
    ul rate ${link}kbit

  # low priority traffic: don't guarantee anything for the first 10 seconds,
  # then guarantee 1/30, until it reaches upper limit (uplink/4)
...
#  for ports_prefs in $VOIP_PORTS; do set_class_by_port $ports_prefs $CLASS_LOW_LATENCY $chain; done
  ...
  # finally rules by l7-proto
  local voip_proto;
  for voip_proto in ${VOIP_PROTOS}
  do
    local layer7_voip_rule="-m layer7 --l7proto ${voip_proto}"
    args=("${layer7_voip_rule}" "$CLASS_LOW_LATENCY" "$chain")
    apply_chain_rules args[@]
  done
#  $IPTABLES 
...
#   for ports_prefs in $INTERACTIVE_PORTS; do set_class_by_port $ports_prefs $CLASS_INTERACTIVE $chain; done
...
#   for ports_prefs in $BROWSING_PORTS; do set_class_by_port $ports_prefs $CLASS_BROWSING $chain; done
  ...
  # rules by ports
#   for ports_prefs in $P2P_PORTS; do 
#     set_class_by_port $ports_prefs $CLASS_LOW_PRIORITY $chain;
#     # reforced
# #    $IPTABLES -t mangle -A $chain -p tcp --sport $port -j MARK --set-mark $CLASS_LOW_PRIORITY
#   done
  set_class_by_ports_prefs "$P2P_PORTS" $CLASS_LOW_PRIORITY $chain
  
  # layer7 for p2p
  local p2p_p;
  for p2p_p in ${P2P_PROTOS}
  do
    local layer_p2p_rule="-m layer7 --l7proto ${p2p_p}"
    args=("${layer_p2p_rule}" "$CLASS_LOW_PRIORITY" "$chain")
    apply_chain_rules args[@]
  done
 
  # remaining short ACK (64) in interactive class
#   local short_ack_rule="-p tcp -m tcp --tcp-flags FIN,SYN,RST,ACK ACK -m length --length :64"
#   args=("${short_ack_rule}" "$CLASS_INTERACTIVE" "${chain}" "tcp");
#   apply_chain_rules args[@]
  
  # filter remaining udp packets for p2p
#  local udp_p2p_source_rule="-p udp ! --sport 53,1190,5060:5061"
#  local udp_p2p_destination_rule="-p udp ! --dport 53,1190,5060:5061"
#  local udp_p2p_rule
#  for udp_p2p_rule in "${udp_p2p_source_rule}" "${udp_p2p_destination_rule}"
#n   local udp_p2p_rule="-p udp "
#n   udp_p2p_rule="" #new
#  do
#n   args=("${udp_p2p_rule}" $CLASS_LOW_PRIORITY $chain "udp")
#n   args=("${udp_p2p_rule}" $CLASS_LOW_PRIORITY $chain ) # new
#   apply_chain_rules args[@]
#  done
...
#    apply_chain_rules args[@]
#    ${IPTABLES} -t mangle -A $chain_pre -m mark ! --mark 0 -j LOG --log-prefix "marked fake with 5 -> "
  }
  
  # finally save
  #c   if [ ! -z $proto ]
#c   then
#c #     echo "we have some proto $proto"
#c     second_rule+=" -p "${proto}
#c   fi
#c   second_rule+=" -m mark --mark $class_mark -j CONNMARK --save-mark"
#c   $second_rule
...
#m  ${IPTABLES} -t mangle -A ${chain} \
  ${IPTABLES} -t mangle -A ${chain_pre} \
    ${rule} \
    -j MARK --set-mark ${class_mark}
#c   local second_rule="${IPTABLES} -t mangle -A $chain_pre"
#c   if [ ! -z $proto ]
#c   then
#c #     echo "we have some proto $proto"
#c     second_rule+=" -p "${proto}
...
#m     ${IPTABLES} -t mangle -A $chain -m mark --mark $class_mark -j LOG --log-prefix "marked with $class_mark -> "
  fi
...
# function set_class_by_port() {
#   
#   [ $DEBUG ] && echo -e "\n\nstart set_class_by_port\n"
# 
#   local prefs_struct=$1
# [ ! -z $DEBUG ] && echo "prefs_struct ${prefs_struct}"
#   local class=$2
# [ ! -z $DEBUG ] && echo "class $class"
#   local shaper_chain=$3
# [ ! -z $DEBUG ] && echo "shaper_chain $shaper_chain"
# #  local shaper_chain_pre="${shaper_chain}_PRE"
#   
#   
#   local -a arr=(${prefs_struct//:/ })
#    #local prs=${arr[0]}
# [ ! -z $DEBUG ] && echo "arr ${arr[@]}"
#   local -a protos=(${arr[0]//,/ })
# [ ! -z $DEBUG ] && echo "PROTOS ${protos[@]}"
#   local port_range=(${arr[1]//-/:})
# [ ! -n $DEBUG ] && echo "port_range $port_range"
#   local direction=${arr[2]}
# [ ! -z $DEBUG ] && echo "direction $direction"
#   
#   local proto
#   for proto in "${protos[@]}"
#   do
# #     echo "proto $proto port_range $port_range direction $direction"
# #   local port_target;
#     local direction_target
#     local ports_rule
#     if [[ $shaper_chain =~ .*EGRESS.* ]]
#     then      
#       if [ -z $direction ]
#       then
# 	for direction_target in "--sport" "--dport"
# 	do
# #	  [ ! -z $DEBUG ] && echo "passing to apply_class_by_port_range: $proto $direction_target $port_range $class $shaper_chain"
# 	  [[ $port_range =~ ".*,.*" ]] && ports_rule="-m multiport ${direction_target}s"
# 	  apply_class_by_port_range $proto $direction_target $port_range $class $shaper_chain
# 	done
#       elif [ $direction == "s" ]
#       then
# 	direction_target="--sport"
# 	apply_class_by_port_range $proto $direction_target $port_range $class $shaper_chain
#       elif [ $direction == "c" ]
#       then
# 	direction_target="--dport"
# 	apply_class_by_port_range $proto $direction_target $port_range $class $shaper_chain    
#       fi
#     elif [[ $shaper_chain =~ .*INGRESS.* ]]
#     then 
# #      if [ -z $direction ] || [ $direction == "i" ]    
# #      then
# #         echo "FOUND INGRESS PORTS"
# 	direction_target="--dport"
# 	apply_class_by_port_range $proto $direction_target $port_range $class $shaper_chain
# #      fi
#     fi
...
# #   do
# #     for port_target in "sport";
# #     #"dport";
# #     do
# #       local rule="-p $proto --$port_target $port"
# #       local args=("${rule}" $class $shaper_chain $proto)
# #       apply_chain_rules args[@]
# #     done   
# #   done
#   [ $DEBUG ] && echo -e "end set_class_by_port"
# }
